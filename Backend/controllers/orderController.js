...
const createEsewaOrder = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Could not create eSewa order" });
  }
};
...
const verifyEsewaPayment = async (req, res) => {
  try {
    ...
    const order = await oredrModel.findOne({
    ...
    if (!order) {
      return res
        ...
        .json({
          success: false,
          message: "No pending order was found for this eSewa transaction",
        });
    }
    ...
    if (order.esewaProductCode !== productCode) {
      return res
        ...
        .json({
          success: false,
          message: "eSewa product code does not match the order",
        });
    }
    ...
    if (!Number.isFinite(totalAmount) || Number(order.amount) !== totalAmount) {
      return res
        ...
        .json({
          success: false,
          message: "eSewa payment amount does not match the order",
        });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Could not verify eSewa payment" });
  }
};
...
const updateStatus = async (req, res) => {
  try {
    await oredrModel.findByIdAndUpdate(req.body.orderId, {
